...
const votingEscrowContract =  new web3.eth.Contract(votingEscrowABI, votingEscrowAddress);
...
module.exports.getBoost = async(vault) => {
  ...
  const gaugeBalance = await gaugeContract.methods.balanceOf(voterAddress).call() / 1e18;
  const gaugeTotal = await gaugeContract.methods.totalSupply().call() / 1e18;
  const workingBalance = await gaugeContract.methods.working_balances(voterAddress).call() / 1e18;
  const workingSupply = await gaugeContract.methods.working_supply().call() / 1e18;
  const vecrvBalance = await votingEscrowContract.methods.balanceOf(voterAddress).call() / 1e18;
  const vecrvTotal = await votingEscrowContract.methods.totalSupply().call() / 1e18;
